...
| high_match_thresh | 0.8 | Cost threshold for linear assignment. Sets the maximum allowable cost for associating detections with existing tracks. A lower threshold enforces stricter matching, which can reduce ID switches between nearby objects (good), but may also lead to more broken tracks (bad). |
